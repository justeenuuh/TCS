<?php
ob_start(); session_start(); require_once('mysqli_connect.php');
 ...
 if(isset($_POST["Import"]))
 {
	        ...
	        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
	         {
	           ...
	           $sql = "INSERT into thesis (code,ttitle,category,year,author,keyword,location,status) values ('".$getData[0]."','".$getData[1]."','".$getData[2]."','".$getData[3]."','".$getData[4]."','".$getData[5]."','".$getData[6]."','".$getData[7]."')";
				$result = @mysqli_query($dbc, $sql);
				if(!isset($result))
				{
					echo "<script type=\"text/javascript\">
							swal(\"Invalid File:Please Upload CSV File.\");
							window.location = \"thesisadmin.php\"
						  </script>";		
				}
				else {
					  echo "<script type=\"text/javascript\">
	...
	if(isset($_POST["Export"]))
	{
	 date_default_timezone_set("ASIA/SINGAPORE");
      ...
      header('Content-Disposition: attachment; filename=' . date('m-d-Y_h:i a') . '_thesislist.csv');  
      $output = fopen("php://output", "w");  
      fputcsv($output, array('Thesis Code', 'Thesis Title', 'Category', 'Year', 'Author', 'Keyword', 'Location', 'Status'));  
